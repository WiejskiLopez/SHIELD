...
def test_all_aggregates_have_factory() -> None:
    ...
    for path in _iter_domain_files():
        ...
        for node in ast.walk(tree):
            ...
            has_factory = any(
                ...
                and (m.name == "open" or m.name == "create" or m.name.startswith("create_"))
                for m in node.body
            )
            if has_factory:
                continue
    ...
    assert not missing, architecture_assertion_message(
        ...
        "AggregateRoots must have a public creation factory (create, create_*, or an established domain name such as open). Add known missing to _KNOWN_NO_FACTORY:\n"
        + "\n".join(missing),
    )
