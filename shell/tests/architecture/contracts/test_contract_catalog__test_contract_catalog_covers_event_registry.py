...
def test_contract_catalog_covers_event_registry() -> None:
    for bounded_context in _CATALOG_MODULES:
        ...
        assert catalog.names() == registered, architecture_assertion_message(
            "reguła testowana przez test_contract_catalog_covers_event_registry",
            "warunek zapisany w asercji musi być spełniony",
            f"{bounded_context}: catalog entries must exactly match registered event types. Missing in catalog: {sorted(registered - catalog.names())}; registered but not in catalog: {sorted(catalog.names() - registered)}",
        )
