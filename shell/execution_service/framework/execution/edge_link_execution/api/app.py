...
def create_edge_link_execution_app(container: ContainerProtocol, *, api_key: str = "") -> FastAPI:
    ...
    if not api_key:
        raise ValueError(
            "create_edge_link_execution_app requires a non-empty api_key (fail-closed)"
        )
