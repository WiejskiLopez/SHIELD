...
class StartNodeExecutionHandler(NodeExecutionTransitionService, CommandHandler[StartNodeExecutionCommand]):
    def __init__(self, unit_of_work: UnitOfWork, clock: Clock) -> None:
        self._unit_of_work = unit_of_work
        self._clock = clock
        super().__init__(unit_of_work, clock)

    async def handle(self, command: StartNodeExecutionCommand) -> None:
        await self.execute(command.node_execution_id, lambda node, now: node.start(now))
