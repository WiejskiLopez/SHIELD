...
class ResumeWorkflowHandler(CommandHandler[ResumeWorkflowCommand]):
    def __init__(self, unit_of_work: UnitOfWork, clock: Clock) -> None:
        self._unit_of_work = unit_of_work
        self._clock = clock

    async def handle(self, command: ResumeWorkflowCommand) -> None:
        async with self._unit_of_work as unit_of_work:
            ...
            workflow.resume(OccurredAt.from_datetime(self._clock.now()))
