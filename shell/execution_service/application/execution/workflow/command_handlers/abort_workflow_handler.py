...
class AbortWorkflowHandler(CommandHandler[AbortWorkflowCommand]):
    def __init__(self, unit_of_work: UnitOfWork, clock: Clock) -> None:
        self._unit_of_work = unit_of_work
        self._clock = clock

    async def handle(self, command: AbortWorkflowCommand) -> None:
        async with self._unit_of_work as unit_of_work:
            ...
            workflow.abort(now=OccurredAt.from_datetime(self._clock.now()))
