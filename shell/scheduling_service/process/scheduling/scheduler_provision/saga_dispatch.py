"""Mapowanie kroków scheduler_provision na kontrakty wire (SAGA.MD Krok 7)."""
...
PROVISION_STEP = StepName("provision_schedule")
RELEASE_STEP = StepName("release_schedule")


class SchedulerProvisionDispatchResolver(StepDispatchResolver):
    ...
    def for_step(
        ...
        if step == RELEASE_STEP:
            return StepDispatch(
                contract_type="scheduling.scheduler_provision.release_schedule",
                destination_service="scheduling",
                payload={"scheduler_job_id": key.business_key, "attempt": attempt},
        ...
        if step == PROVISION_STEP:
            ...
            return StepDispatch(
                contract_type="scheduling.scheduler_provision.provision_schedule",
                destination_service="scheduling",
                payload={
                    "scheduler_job_id": key.business_key,
                    "fail": raw_fail,
                    "attempt": attempt,
                },
                aggregate_id=key.business_key,
